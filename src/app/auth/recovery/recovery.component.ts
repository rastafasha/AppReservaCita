...
export class RecoveryComponent implements OnInit {
  ...
  recovery(recoveryForm: { valid: any; }){
    if(recoveryForm.valid){
      this.userService.set_recovery_token(this.email).subscribe(
        response =>{
  ...
  verify(verifyForm: { valid: any; }){
    if(verifyForm.valid){
      this.userService.verify_token(this.email, this.codigo).subscribe(
        response =>{
          console.log(response);
  ...
  change_password(passwordForm: { valid: any; }){debugger
    if(passwordForm.valid){
      if(this.new_password.length <= 7){
        ...
        this.userService.change_password(this.email,{password:this.new_password}).subscribe(
          response =>{
            console.log(response);
